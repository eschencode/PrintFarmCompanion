<script lang="ts">
    ...
    const STATUS_RANK: Record<SpoolStatus, number> = {
        empty: 0,
        low: 1,
        ok: 2,
    };
    ...
    const enriched = $derived<EnrichedPreset[]>(
        (data.spoolPresets || [])
            .map((preset) => {
                ...
                const { status, daysLeft } = computeDepletion(
                    preset.in_storage ?? 0,
                    used30d,
                );
    ...
    const totalSpools = $derived(
        (data.spoolPresets || []).reduce(
            (sum, p) => sum + (p.in_storage || 0),
            0,
        ),
    );
    // Spool demand for the next 30 days, derived from the global print queue
    // (capped to a 30-day cover server-side). preset_id -> demand row.
    const demandByPreset = $derived(
        new Map((data.spoolDemand || []).map((d) => [Number(d.preset_id), d])),
    );

    // Reorder plan: spools the 30-day queue drains past what's on hand.
    const reorderPlan = $derived(
        (data.spoolDemand || []).filter((d) => d.grams_deficit > 0),
    );
    const totalToOrder = $derived(
        reorderPlan.reduce((sum, d) => sum + d.spools_to_buy, 0),
    );
    // Total filament the next 30 days of queue will consume (for the metric card).
    const totalNeeded30d = $derived(
        (data.spoolDemand || []).reduce((sum, d) => sum + d.grams_needed, 0),
    );
    // Filament consumed in the last 30 days, in grams (used_30d is a spool count).
    const totalUsed30dGrams = $derived(
        enriched.reduce(
            (sum, e) => sum + e.used30d * (e.preset.default_weight || 1000),
            0,
        ),
    );
    const kg = (grams: number) => (grams / 1000).toFixed(1);

    // Projected spools left for a preset after fulfilling the 30-day queue.
    // Presets with no queued demand are untouched (current stock stands).
    function projectedSpoolsLeft(e: EnrichedPreset): number {
        const d = demandByPreset.get(Number(e.preset.id));
        if (!d) return e.preset.in_storage ?? 0;
        const weight = e.preset.default_weight || 1000;
        return (d.grams_available - d.grams_needed) / weight;
    }
...
<div
    class="min-h-screen bg-white dark:bg-[#0a0a0a] text-zinc-900 dark:text-zinc-50 p-6"
>
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            <div
                class="bg-zinc-50 dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg p-6"
            >
                <p class="text-zinc-500 text-sm mb-1">In storage</p>
                <p
                    class="text-3xl font-medium text-zinc-900 dark:text-zinc-50 tabular-nums"
                >
                    {totalSpools}
                </p>
            </div>
            <div
                class="bg-zinc-50 dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg p-6"
            >
                <p class="text-zinc-500 text-sm mb-1">Used last 30 days</p>
                <p class="text-3xl font-medium text-purple-400 tabular-nums">
                    {kg(totalUsed30dGrams)}<span
                        class="text-xl text-zinc-400 dark:text-zinc-600"
                        >kg</span
                    >
                </p>
            </div>
            <div
                class="bg-zinc-50 dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg p-6"
            >
                <p class="text-zinc-500 text-sm mb-1">To order</p>
                <p
                    class="text-3xl font-medium tabular-nums {totalToOrder > 0
            ...
            <div
                class="bg-zinc-50 dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg p-6"
            >
                <p class="text-zinc-500 text-sm mb-1">Needed next 30 days</p>
                <p
                    class="text-3xl font-medium text-blue-600 dark:text-blue-400 tabular-nums"
                >
                    {kg(totalNeeded30d)}<span
                        class="text-xl text-zinc-400 dark:text-zinc-600"
                        >kg</span
                    >
                </p>
        ...
        <!-- Order plan · next 30 days (driven by the print queue) -->
        <div
            class="bg-zinc-50 dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg p-6 mb-6"
        >
            <div class="flex items-center justify-between mb-5">
                ...
                {#if totalToOrder > 0}
                    <span
                        class="text-xs font-semibold text-red-600 dark:text-red-500 tabular-nums shrink-0 bg-red-500/10 px-2.5 py-1 rounded-md"
                        >{totalToOrder} to order</span
                    >
                {/if}
            </div>
            ...
            {#if reorderPlan.length === 0}
                ...
                >
                    <svg
                        ...
                        /></svg
                    >
                    Fully stocked for the next 30 days of the print queue.
                </p>
            {:else}
                <div class="space-y-2">
                    {#each reorderPlan as d (d.preset_id)}
                        <div
                            ...
                            <span class="flex items-center gap-2 min-w-0">
                                {#if d.color_hex}
                                    <span
                                        ...
                                        style="background-color: {d.color_hex}"
                                    ></span>
                                {/if}
                                <span
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                ...
                                >
                                    ...
                                    <input
                                        type="hidden"
                                        name="quantity"
                                        value={d.spools_to_buy}
                                    />
                                    <button
                                        type="submit"
                                        title="Mark {d.spools_to_buy} received — adds to storage"
                                        class="px-2.5 h-7 flex items-center rounded-md bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-zinc-200 dark:text-zinc-900 text-white text-xs font-medium tabular-nums transition-colors"
                                    >
                                        +{d.spools_to_buy} stock
                                    </button>
                                </form>
                            </div>
                        </div>
        ...
        <!-- Spool List -->
        <div
            class="bg-zinc-50 dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg overflow-hidden"
        >
            <div
                class="px-6 py-4 border-b border-zinc-200 dark:border-[#262626] flex items-center justify-between"
            >
                <p
                    class="text-xs font-medium text-zinc-400 dark:text-zinc-600 tracking-wide uppercase"
                >
                ...
                <span
                    class="text-[11px] text-zinc-400 dark:text-zinc-600 tracking-wide uppercase"
                    >used 30d · left in 30d</span
                >
            </div>
            ...
            {#if enriched.length === 0}
                <div class="p-8 text-center text-zinc-400 dark:text-zinc-500">
                    <p>
                        No spools yet. Add one to start tracking your inventory.
                    </p>
                </div>
            {:else}
                <div class="divide-y divide-zinc-200 dark:divide-[#262626]">
                    {#each enriched as e (e.preset.id)}
                        ...
                        {@const left = projectedSpoolsLeft(e)}
                        <div
                            class="flex items-center gap-4 px-6 py-3 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors {(preset.in_storage ||
                                0) === 0
                                ? 'opacity-50'
                                : ''}"
                        >
                            ...
                            <div class="relative shrink-0">
                                ...
                                {:else}
                                    <div
                                        class="w-3.5 h-3.5 rounded-full {colorClass(
                                            preset.color,
                                        )}"
                                    ></div>
                                {/if}
                                <span
                                    class="absolute -top-1 -right-1 w-2 h-2 rounded-full ring-1 ring-white dark:ring-[#111111] {STATUS_DOT[
                                        e.status
                                    ]}"
                                    title="{STATUS_LABEL[
                                        e.status
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="font-medium text-zinc-900 dark:text-zinc-50 truncate text-sm"
                                        >{presetName(preset)}</span
                                    >
                                    <span
                                        class="text-xs text-zinc-500 bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded-sm"
                                        >{preset.material}</span
                                    >
                                </div>
                                <div
                                    class="text-xs text-zinc-400 dark:text-zinc-500 truncate"
                                >
                                    {#if preset.cost}${preset.cost.toFixed(2)} ·
                                    {/if}{preset.default_weight}g spool
                                </div>
                            </div>

                            <!-- Used in the last 30 days (spool count) -->
                            <div class="text-right shrink-0 tabular-nums w-16">
                                <div
                                ...
                                <div
                                    class="text-[10px] text-zinc-400 dark:text-zinc-600 uppercase tracking-wide"
                                >
                                    used 30d
                                </div>
                            </div>

                            <!-- Projected stock left after the next 30 days of the queue -->
                            <div class="text-right shrink-0 tabular-nums w-16">
                                <div
                                    class="text-sm {left < 0
                                        ? 'text-red-600 dark:text-red-500'
                                        : left < 1
                                          ? 'text-amber-600 dark:text-amber-500'
                                          : 'text-zinc-700 dark:text-zinc-300'}"
                                >
                                    {left < 0 ? "0" : left.toFixed(1)}
                                </div>
                                <div
                                    class="text-[10px] text-zinc-400 dark:text-zinc-600 uppercase tracking-wide"
                                >
                            ...
                            {:else}
                                <div class="flex items-center gap-1">
                                    ...
                                    <button
                                        ...
                                        class="w-10 h-7 flex items-center justify-center rounded-sm bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-sm font-medium {(preset.in_storage ||
                                            0) === 0
                                            ? 'text-zinc-300'
                                            : (preset.in_storage || 0) <= 2
                                              ? 'text-amber-600'
                                              : 'text-zinc-900 dark:text-zinc-50'} transition-colors"
                                    >
                                        {preset.in_storage || 0}
                                    </button>
        ...
        <!-- Quick add stock (utility — receive a delivery without the per-row steppers) -->
        <div
            class="bg-zinc-50 dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg p-6 mt-6"
        >
            <p
                class="text-xs font-medium text-zinc-400 dark:text-zinc-600 tracking-wide uppercase mb-4"
            >
                Quick Add Stock
            </p>
            <form
            ...
            >
                <div class="flex-1 min-w-[200px]">
                    <label
                        for="storage-preset-id"
                        class="block text-xs text-zinc-500 mb-1">Spool</label
                    >
                    <select
                    ...
                    >
                        ...
                        {#each enriched as { preset }}
                            <option value={preset.id}>
                                {presetName(preset)} ({preset.material}) - {preset.in_storage ||
                                    0} in stock
                            </option>
                        {/each}
                    </select>
                ...
                <div class="w-24">
                    <label
                        for="storage-qty"
                        class="block text-xs text-zinc-500 mb-1">Quantity</label
                    >
                    <input
                        id="storage-qty"
                        type="number"
...
{#if showAddPresetModal}
    ...
    >
        <div
            class="bg-white dark:bg-[#111111] border border-zinc-200 dark:border-[#262626] rounded-lg w-full max-w-md"
        >
            <div
                class="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-[#262626]"
            >
                <h2
                    class="text-base font-medium text-zinc-900 dark:text-zinc-50"
                >
                    New Spool
                </h2>
                <button
