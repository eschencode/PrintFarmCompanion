<script lang="ts">
    ...
    // extend item type locally to include AI + production fields
    ...
    // Days of lead time assumed before a reprint lands as sellable stock. Drives
    // the "reorder now" KPI and the per-row reorder-by date.
    ...
    function getCountFromWeight(idKey: string): number {
        ...
        const unitWeight = (data.unitWeights || []).find(
            (w) => String(w.id) === idKey,
        );
        if (!unitWeight || unitWeight.weight_per_unit <= 0 || weight <= 0)
            return 0;
    ...
    const tree = $derived.by(() => {
        ...
        for (const it of items) {
            if (it.category_id != null && byId.has(it.category_id))
                pushTo(itemsByCat, it.category_id, it);
        ...
        for (const c of cats) {
            if (c.parent_id != null) {
                ...
                if (arr) arr.push(c);
                else subsByParent.set(c.parent_id, [c]);
            }
        }
        ...
        const groups: CatGroup[] = cats
            ...
            .map((cat) => {
                ...
                const subs: SubGroup[] = (subsByParent.get(cat.id) ?? []).map(
                    (sc) => {
                ...
                return {
                    cat,
                    items: sortItems(directItems),
                    subs,
                    stats: statsOf(allItems),
                };
            });
        ...
        return {
            ...
            uncatStats: statsOf(uncategorized),
        };
    });
    ...
    async function submitRename(id: number) {
        if (catNameDraft.trim())
            await postAction("?/renameCategory", {
                id,
    ...
    function needsReorder(i: ObjectItemUI): boolean {
        const vel = i.daily_velocity ?? 0;
        if (vel <= 0) return false;
        if (i.in_stock <= 0) return true;
        return (i.days_until_stockout ?? 999) <= LEAD_TIME_DAYS;
    }
    const reorderNowCount = $derived(
        (data.items || []).filter(needsReorder).length,
    );
    const weeklyThroughput = $derived(data.weeklyThroughput ?? 0);

    // ── Global print queue (the real backlog, shared with printer assignment) ────
    // This is the single source of truth — the same `print_queue` table the
    // dashboard knapsacks per printer. The "Print queue" panel renders THIS, not a
    // client-side recompute, so inventory and per-printer views never diverge.
    const globalQueue = $derived<PrintQueueItem[]>(data.globalQueue ?? []);
    // Counts whose last manual count is missing or older than 30 days — i.e. numbers
    // you probably shouldn't fully trust. last_count_date is Unix seconds.
    const staleCount = $derived.by(() => {
        const cutoff = Date.now() / 1000 - 30 * 86_400;
        return (data.items || []).filter(
            (i) => i.last_count_date == null || i.last_count_date < cutoff,
        ).length;
    });

    // Lower = more urgent. Out-of-stock floats to the top even without sales data;
    // then imminent stockouts, then low stock, then everything healthy.
    function urgencyTier(i: ObjectItemUI): number {
        if (i.in_stock <= 0) return 0;
        if (
            (i.daily_velocity ?? 0) > 0 &&
            (i.days_until_stockout ?? 999) <= LEAD_TIME_DAYS
        )
            return 1;
        if (i.min_threshold > 0 && i.in_stock < i.min_threshold) return 2;
        if ((i.daily_velocity ?? 0) > 0) return 3;
        return 4;
    }
    // Sort a list of items by the active sort mode.
    function sortItems(items: ObjectItemUI[]): ObjectItemUI[] {
        if (sortMode === "name")
            return [...items].sort((a, b) => a.name.localeCompare(b.name));
        // Urgency: by tier, then soonest stockout, then name.
        return [...items].sort((a, b) => {
            const at = urgencyTier(a),
                bt = urgencyTier(b);
            if (at !== bt) return at - bt;
            const ad = a.days_until_stockout ?? 999;
            const bd = b.days_until_stockout ?? 999;
            if (ad !== bd) return ad - bd;
            return a.name.localeCompare(b.name);
        });
    }

    // Demand trend: 7d sell-through vs the 30d-implied weekly rate.
    function trendOf(item: ObjectItemUI): "up" | "down" | "flat" | null {
        const s7 = item.sold_7d ?? 0;
        const s30 = item.sold_30d ?? 0;
        if (s30 < 3) return null; // too little data to call a trend
        const expected7 = (s30 / 30) * 7;
        if (s7 >= expected7 * 1.25) return "up";
        if (s7 <= expected7 * 0.75) return "down";
        return "flat";
    }
    ...
    function getChangeTypeLabel(type: string): {
        ...
        switch (type) {
            ...
            case "- sold b2c":
                return { label: "B2C", color: "text-blue-600" };
            case "- sold b2b":
                return { label: "B2B", color: "text-purple-400" };
            default:
    ...
    // Cover beyond a year (or no velocity at all) is "effectively unlimited" — show
    // a flat 365+ instead of huge numbers or ∞.
    function formatDays(d?: number) {
        if (d == null || d >= 365) return "365+d";
    ...
    const activityGroups = $derived(() => {
        ...
        for (const log of data.logs || []) {
            ...
            current = {
                orderId: oid,
                orderNumber: log.order_number ?? null,
...
<div class="min-h-screen bg-[#f8f8f8] dark:bg-[#080808]">
    <div class="max-w-6xl mx-auto px-6 sm:px-8 py-10">
        ...
        <div class="flex items-start justify-between mb-10">
            <div>
                <h1
                    class="text-[2rem] font-semibold tracking-tight text-zinc-900 dark:text-zinc-50 leading-none"
                >
                    Inventory
                </h1>
                <p class="text-zinc-400 dark:text-zinc-500 text-sm mt-2">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
            ...
            >
                <p
                    class="text-[10px] font-semibold uppercase tracking-[0.12em] mb-3 {reorderNowCount >
                    0
                ...
                <p
                    class="text-4xl font-light tabular-nums {reorderNowCount > 0
                        ? 'text-red-500 dark:text-red-400'
            ...
            >
                <p
                    class="text-[10px] font-semibold uppercase tracking-[0.12em] mb-3 {runoutWeek.length >
                    0
                ...
                <p
                    class="text-4xl font-light tabular-nums {runoutWeek.length >
                    0
                        ? 'text-amber-500 dark:text-amber-400'
                        : 'text-zinc-900 dark:text-zinc-50'}"
                >
                ...
                <p class="text-[10px] text-zinc-400 dark:text-zinc-600 mt-1">
                    at current sell-through
                </p>
            </div>
            <div
                class="bg-white dark:bg-[#111] rounded-xl border border-zinc-100 dark:border-[#1e1e1e] p-5"
            >
                <p
                    class="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-400 dark:text-zinc-500 mb-3"
                >
                    Sold / 7d
                </p>
                <p
                    class="text-4xl font-light tabular-nums text-zinc-900 dark:text-zinc-50"
                >
                    {weeklyThroughput}
                </p>
                <p class="text-[10px] text-zinc-400 dark:text-zinc-600 mt-1">
            ...
            <div
                class="rounded-xl border p-5 transition-colors {staleCount > 0
                    ? 'bg-white dark:bg-[#111] border-zinc-200 dark:border-zinc-700'
                    : 'bg-white dark:bg-[#111] border-zinc-100 dark:border-[#1e1e1e]'}"
            >
                <p
                    class="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-400 dark:text-zinc-500 mb-3"
                ...
                <p
                    class="text-4xl font-light tabular-nums {staleCount > 0
        ...
        >
            ...
            {#if lowStockCount > 0}
                <span class="text-zinc-200 dark:text-zinc-800">·</span>
                <span class="text-amber-500/80">{lowStockCount} low</span>
            {/if}
            {#if outOfStockCount > 0}
                <span class="text-zinc-200 dark:text-zinc-800">·</span>
                <span class="text-red-500/80">{outOfStockCount} out</span>
            {/if}
        </div>
        ...
        <div class="flex items-center gap-3 mb-6">
            <div class="relative flex-1 max-w-sm">
                <svg
                    class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-400"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                >
            ...
            <label
                class="flex items-center gap-2 h-9 px-3.5 rounded-lg border border-zinc-200 dark:border-[#1e1e1e] bg-white dark:bg-[#111] cursor-pointer hover:bg-zinc-50 dark:hover:bg-[#161616] transition-colors select-none"
            >
                <input
                    type="checkbox"
                ...
                />
                <span
                    class="text-xs font-medium text-zinc-500 dark:text-zinc-400 whitespace-nowrap"
                    >Low stock only</span
                >
            </label>
            ...
            <!-- Sort toggle -->
            <div
                class="flex items-center h-9 rounded-lg border border-zinc-200 dark:border-[#1e1e1e] bg-white dark:bg-[#111] overflow-hidden"
            >
                <button
                    onclick={() => (sortMode = "urgency")}
                    class="h-full px-3 text-xs font-medium transition-colors {sortMode ===
                    'urgency'
                        ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900'
                        : 'text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-[#161616]'}"
                    >Urgency</button
                >
                <button
        ...
        <div class="grid lg:grid-cols-3 gap-5 mb-10">
            ...
            <div class="lg:col-span-2">
                ...
                >
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            {#if !showReorderList}
                                <button
                                    ...
                                    class="w-7 h-7 flex items-center justify-center rounded-md transition-colors {manageMode
                                        ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900'
                    ...
                    {#if manageMode}
                        <div
                            class="px-5 py-2 bg-blue-50/60 dark:bg-blue-950/20 border-b border-blue-100/60 dark:border-blue-900/30 text-[11px] text-blue-600 dark:text-blue-300"
                        >
                            Organizing — drag items into categories,
                            double-click a name to rename, or use the +/trash
                            actions.
                        </div>
                    {/if}
                        ...
                        {:else}
                            ...
                            {#each tree.groups as g (g.cat.id)}
                                ...
                                <div
                                    ...
                                    class="transition-colors {dragOverKey ===
                                    catKey
                                        ? 'bg-blue-50/60 dark:bg-blue-950/20 ring-1 ring-inset ring-blue-400/50'
                                        : ''}"
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            <svg
                                                class="w-3 h-3 shrink-0 text-zinc-300 dark:text-zinc-600 transition-transform duration-200 {expanded
                                                    ? 'rotate-90'
                                                    : ''}"
                                                fill="none"
                                                stroke="currentColor"
                                        ...
                                        >
                                            {#if g.stats.out > 0}
                                                <span
                                                    ...
                                                    ><span
                                                        class="w-1.5 h-1.5 rounded-full bg-red-400 inline-block"
                                                    ></span>{g.stats.out} out</span
                                                >
                                            {/if}
                                            {#if g.stats.low > 0}
                                                <span
                                                    class="text-[10px] font-medium text-amber-500 flex items-center gap-1"
                                                    ><span
                                                        class="w-1.5 h-1.5 rounded-full bg-amber-400 inline-block"
                                                    ></span>{g.stats.low} low</span
                                                >
                                            {/if}
                                            <span
                                                class="text-xl font-light tabular-nums text-zinc-600 dark:text-zinc-400 min-w-12 text-right"
                                                >{g.stats.totalStock}</span
                                            >
                                            {#if manageMode}
                                                <div
                                                ...
                                                >
                                                    <button
                                                        ...
                                                        class="w-6 h-6 flex items-center justify-center rounded-md text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 dark:hover:bg-zinc-800"
                                                    >
                                                        <svg
                                                            class="w-3.5 h-3.5"
                                                            fill="none"
                                                    ...
                                                    <button
                                                        ...
                                                        class="w-6 h-6 flex items-center justify-center rounded-md text-zinc-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30"
                                                    >
                                                        <svg
                                                            class="w-3.5 h-3.5"
                                                            fill="none"
                                    ...
                                    {#if expanded}
                                        <div
                                            class="border-t border-zinc-50 dark:border-[#171717]"
                                        >
                                            ...
                                            {#each g.subs as s (s.cat.id)}
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <svg
                                                                class="w-2.5 h-2.5 shrink-0 text-zinc-300 dark:text-zinc-600 transition-transform duration-200 {subExpanded
                                                                    ? 'rotate-90'
                                                                    : ''}"
                                                                fill="none"
                                                                stroke="currentColor"
                                                        ...
                                                        <div
                                                            class="flex items-center gap-3 shrink-0"
                                                        >
                                                            {#if s.stats.out > 0}<span
                                                                    class="text-[10px] text-red-500"
                                                                    >{s.stats
                                                                        .out} out</span
                                                                >{/if}
                                                            {#if s.stats.low > 0}<span
                                                                    class="text-[10px] text-amber-500"
                                                                    >{s.stats
                                                                        .low} low</span
                                                                >{/if}
                                                            <span
                                                                class="text-base font-light tabular-nums text-zinc-500 min-w-8 text-right"
                                                                >{s.stats
                                                                    .totalStock}</span
                                                            >
                                                            {#if manageMode}
                                                                <button
                                                                    ...
                                                                    class="w-5 h-5 flex items-center justify-center rounded text-zinc-400 hover:text-red-500"
                                                                >
                                                                    <svg
                                                                        class="w-3 h-3"
                                                                        fill="none"
                                                    ...
                                                    {#if subExpanded}
                                                        ...
                                                        >
                                                            {#if manageMode && s.items.length === 0}
                                                                <div
                                                                    class="pl-16 pr-5 py-3 text-[11px] text-zinc-300 dark:text-zinc-600 italic"
                                                                >
                                                                    Drag items
                                                                    here
                                                                </div>
                                            ...
                                            {#if manageMode && g.items.length === 0 && g.subs.length === 0}
                                                <div
                                                    class="pl-10 pr-5 py-3 text-[11px] text-zinc-300 dark:text-zinc-600 italic"
                                                >
                                                    Drag items here, or add a
                                                    subcategory
                                                </div>
                                            {/if}
                                            ...
                                            {#if manageMode && addingSubFor === g.cat.id}
                                                <div
                                                    class="pl-10 pr-5 py-2.5 flex items-center gap-2 bg-zinc-50/50 dark:bg-[#0d0d0d]"
                                                >
                                                    <!-- svelte-ignore a11y_autofocus -->
                                                    <input
                                                    ...
                                                    <button
                                                        onclick={() =>
                                                            createSub(g.cat.id)}
                                                        class="h-7 px-3 text-xs rounded-md bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hover:opacity-90"
                                                        >Add</button
                                                    >
                                                </div>
                                            {/if}
                            ...
                            {#if manageMode}
                                <div
                                    class="px-5 py-3 flex items-center gap-2 bg-zinc-50/40 dark:bg-[#0d0d0d]"
                                >
                                    <input
                                    ...
                                    <button
                                        ...
                                        class="h-8 px-3.5 text-sm font-medium rounded-lg bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hover:opacity-90 transition-opacity"
                                        >Add category</button
                                    >
                                </div>
                            {/if}
            ...
            <div class="lg:col-span-1">
                ...
                >
                    <div
                        class="px-5 py-4 border-b border-zinc-50 dark:border-[#1a1a1a]"
                    >
                        <h2
                            class="text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-400 dark:text-zinc-500"
                        >
                    ...
                    {:else}
                        <div
                            class="divide-y divide-zinc-50 dark:divide-[#171717] max-h-130 overflow-y-auto"
                        >
                            {#each activityGroups() as group}
                                {#if group.orderId}
                                    ...
                                    <div
                                        class="bg-zinc-50/60 dark:bg-[#0d0d0d]"
                                    >
                                        <div
                                            class="px-5 pt-3 pb-1.5 flex items-center justify-between gap-3"
                                        >
                                            <div
                                                class="flex items-center gap-2 min-w-0"
                                            >
                                                <span
                                                    class="inline-flex items-center justify-center px-1.5 h-4 rounded bg-blue-500/10 text-[9px] font-bold text-blue-500 shrink-0"
                                                    >B2C</span
                                                >
                                                <span
                                                    class="text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 truncate"
                                                >
                                                    {group.orderNumber
                                                ...
                                                <span
                                                    class="text-zinc-300 dark:text-zinc-700 shrink-0"
                                                    >·</span
                                                >
                                                <span
                                            ...
                                            <span
                                                class="text-[11px] font-semibold tabular-nums text-red-500 shrink-0"
                                                >−{orderQty}</span
                                            >
                                        </div>
                                        <div class="pb-2">
                                            {#each group.items as log}
                                                <div
                                                    class="pl-9 pr-5 py-1 flex items-center justify-between"
                                                >
                                                    <span
                                                        class="text-xs text-zinc-600 dark:text-zinc-400 truncate"
                                                        >{log.object_name}</span
                                                    >
                                                    <span
                                                        class="text-xs font-medium tabular-nums text-red-400 shrink-0"
                                                        >−{Math.abs(
                                                            log.quantity,
                                                        )}</span
                                                    >
                                ...
                                {:else}
                                    ...
                                    {@const isOutflow =
                                        log.change_type.startsWith("-")}
                                    <div
                                        class="px-5 py-3 hover:bg-zinc-50 dark:hover:bg-[#161616] transition-colors"
                                    >
                                        <div class="flex items-center gap-3">
                                            <span
                                                class="inline-flex items-center justify-center w-5 h-5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold shrink-0 {changeType.color}"
                                            >
                                                {changeType.label}
                                            </span>
                                            <div class="flex-1 min-w-0">
                                                <p
                                                    class="text-xs font-medium text-zinc-700 dark:text-zinc-300 truncate leading-snug"
                                                >
                                                    {log.object_name}
                                                </p>
                                                <p
                                                    class="text-[10px] text-zinc-400 dark:text-zinc-600 mt-0.5"
                                                >
                                                    {formatTime(log.created_at)}
                                                </p>
                                            </div>
                                            <span
                                                class="text-xs font-semibold tabular-nums shrink-0 {isOutflow
                                                    ? 'text-red-500'
                                                    : 'text-emerald-500'}"
                                            >
                                                {isOutflow
        ...
        >
            ...
            <div class="flex border-b border-zinc-50 dark:border-[#1a1a1a]">
                {#each [{ id: "sets" as const, label: "By Sets", desc: "Decompose bundles" }, { id: "weight" as const, label: "By Weight", desc: "Weight → count" }, { id: "direct" as const, label: "Individually", desc: "Direct count" }] as tab}
                    <button
                        onclick={() => openCheckPanel(tab.id)}
                        class="flex-1 px-5 py-3.5 text-left border-b-2 transition-all duration-150 {activeCheckPanel ===
                        tab.id
                            ? 'border-zinc-900 dark:border-zinc-100'
                            : 'border-transparent hover:bg-zinc-50 dark:hover:bg-[#161616]'}"
                    >
                        <p
                            class="text-xs font-semibold transition-colors {activeCheckPanel ===
                            tab.id
                                ? 'text-zinc-900 dark:text-zinc-50'
                                : 'text-zinc-400 dark:text-zinc-500'}"
                        >
                        ...
                        <p
                            class="text-[10px] text-zinc-400 mt-0.5 hidden sm:block"
                        >
                            {tab.desc}
                        </p>
                    </button>
                ...
                <div class="p-5">
                    ...
                    {#if activeCheckPanel === "sets"}
                        ...
                        {#if filteredSets().length === 0}
                            <p class="text-xs text-zinc-400 text-center py-8">
                                No set definitions found
                            </p>
                        {:else}
                            <div class="grid sm:grid-cols-2 gap-2">
                                {#each filteredSets() as set}
                                    <div
                                        class="rounded-lg border border-zinc-100 dark:border-[#1e1e1e] bg-zinc-50 dark:bg-[#0d0d0d] px-4 py-3"
                                    >
                                        <div
                                        ...
                                        >
                                            <div class="flex-1 min-w-0">
                                                <p
                                                    class="text-xs font-semibold text-zinc-800 dark:text-zinc-200 truncate"
                                                >
                                                    {set.label}
                                                </p>
                                                <p
                                                ...
                                                >
                                                    {set.components
                                                        .map(
                                                            (c) =>
                                                                `${c.quantity}× ${getItemName(String(c.object_id))}`,
                                                        )
                                                        .join(" + ")}
                                                </p>
                                            </div>
                                            ...
                                            >
                                                <button
                                                    ...
                                                    class="w-7 h-7 flex items-center justify-center rounded-md border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                                    >−</button
                                                >
                                                <input
                                                    type="number"
                                                ...
                                                <button
                                                    ...
                                                    class="w-7 h-7 flex items-center justify-center rounded-md bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-zinc-700 dark:hover:bg-zinc-300 transition-colors"
                                                    >+</button
                                                >
                                            </div>
                                        </div>
                        ...
                        {#if Object.keys(setPendingSummary()).length > 0}
                            <div
                                class="mt-4 rounded-lg border border-emerald-100 dark:border-emerald-900/40 bg-emerald-50 dark:bg-emerald-950/20 p-3"
                            >
                                <p
                                    class="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wide mb-2"
                                >
                                    Will add to inventory
                                </p>
                                <div
                                    class="grid grid-cols-2 sm:grid-cols-3 gap-1"
                                >
                    ...
                    {:else if activeCheckPanel === "weight"}
                        <div class="mb-4">
                            ...
                            <p class="text-[10px] text-zinc-400 mt-2">
                                Enter total weight in grams — item count
                                calculated automatically
                            </p>
                        </div>
                        {#if filteredWeightItems().length === 0}
                            <p class="text-xs text-zinc-400 text-center py-8">
                                No items with weight data available
                            </p>
                        {:else}
                            <div class="grid sm:grid-cols-2 gap-2">
                                        ...
                                        >
                                            ...
                                            >
                                                <div class="relative">
                                                    ...
                                                    />
                                                    <span
                                                        class="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-zinc-400"
                                                        >g</span
                                                    >
                                                </div>
                                                <span
                                                ...
                                                >
                                                <span
                                                    class="w-8 text-center text-xs font-semibold tabular-nums {count >
                                                    0
                                                        ? 'text-blue-500'
                                                        : 'text-zinc-400'}"
                                                    >{count}</span
                        ...
                        {#if Object.keys(weightPendingSummary()).length > 0}
                            <div
                                class="mt-4 rounded-lg border border-blue-100 dark:border-blue-900/40 bg-blue-50 dark:bg-blue-950/20 p-3"
                            >
                                <p
                                    class="text-[10px] font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-2"
                                >
                                    Will add to inventory
                                </p>
                                <div
                    ...
                    {:else if activeCheckPanel === "direct"}
                                        ...
                                        >
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p
                                                    class="text-[10px] text-zinc-400 mt-0.5"
                                                >
                                                    {currentStock}{pending > 0
                                                        ? ` → ${currentStock + pending}`
                                                        : ""}
                                                </p>
                                            </div>
                                            <div
                        ...
                        {#if Object.keys(directPendingSummary()).length > 0}
                            <div
                                class="mt-4 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 p-3"
                            >
                                <p
                                    class="text-[10px] font-semibold text-zinc-500 uppercase tracking-wide mb-2"
                                >
                                    Will add to inventory
                                </p>
                                <div
                    ...
                    <!-- Footer -->
                    <div
                        class="mt-5 pt-4 border-t border-zinc-100 dark:border-[#1e1e1e] flex items-center justify-between"
                    >
                        {#if checkMessage}
                        ...
                        {#if activeCheckPanel === "sets"}
                            ...
                            >
                                ...
                                >
                                    {checkSubmitting
                                        ? "Adding…"
                                        : `Add ${Object.values(setPendingSummary()).reduce((s, v) => s + v, 0)} items`}
                                </button>
                            </form>
                        {:else if activeCheckPanel === "weight"}
                            <form
                            ...
                            >
                                ...
                                <button
                                    type="submit"
                                    disabled={checkSubmitting ||
                                        Object.keys(weightPendingSummary())
                                            .length === 0}
                                    class="h-8 px-5 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-lg text-xs font-semibold hover:bg-zinc-800 dark:hover:bg-zinc-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                                >
                                    {checkSubmitting
                                        ? "Adding…"
                                        : `Add ${Object.values(weightPendingSummary()).reduce((s, v) => s + v, 0)} items`}
                                </button>
                            </form>
                        {:else if activeCheckPanel === "direct"}
                            <form
                            ...
                            >
                                ...
                                <button
                                    type="submit"
                                    disabled={checkSubmitting ||
                                        Object.keys(directPendingSummary())
                                            .length === 0}
                                    class="h-8 px-5 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-lg text-xs font-semibold hover:bg-zinc-800 dark:hover:bg-zinc-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                                >
                                    {checkSubmitting
                                        ? "Adding…"
                                        : `Add ${Object.values(directPendingSummary()).reduce((s, v) => s + v, 0)} items`}
                                </button>
                            </form>
                        {/if}
                    </div>
            ...
            {:else}
                <div class="px-5 py-6 text-center">
                    <p class="text-xs text-zinc-400">
                        Select a method above to add inventory
                    </p>
                </div>
            {/if}
        </div>
