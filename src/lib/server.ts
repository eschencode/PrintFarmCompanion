...
// Printer Models
export async function getAllPrinterModels(
  db: D1Database,
...
export async function updatePrinterModel(
  ...
  try {
    ...
    if (model.description !== undefined)
      updates.push(sql`description = ${model.description}`);
    if (model.buildVolumeX !== undefined)
      updates.push(sql`build_volume_x = ${model.buildVolumeX}`);
    if (model.buildVolumeY !== undefined)
      updates.push(sql`build_volume_y = ${model.buildVolumeY}`);
    if (model.buildVolumeZ !== undefined)
      updates.push(sql`build_volume_z = ${model.buildVolumeZ}`);
    if (updates.length === 0)
      return { success: false, error: "No updates provided" };
    await drizzleDb.run(
      sql`UPDATE printer_models SET ${sql.join(updates, sql`, `)} WHERE id = ${id}`,
    );
...
export async function deletePrinterModel(
  db: D1Database,
  id: number,
): Promise<ServerResponse> {
  const drizzleDb = getDb(db);
  try {
    ...
    if ((printerCount?.count || 0) > 0 || (moduleCount?.count || 0) > 0) {
      return {
        success: false,
        error:
          "Cannot delete: printer model is still referenced by printers or modules",
      };
    }
...
export async function updatePrinterStatus(
  ...
  id: number | null,
  status: string,
  loadedSpoolId?: number | null,
): Promise<void> {
  if (!id) return; // Handle null printer_id
  const drizzleDb = getDb(db);
  if (loadedSpoolId !== undefined) {
    await drizzleDb.run(
      sql`UPDATE printers SET status = ${status}, loaded_spool_id = ${loadedSpoolId} WHERE id = ${id}`,
    );
  } else {
    await drizzleDb.run(
...
export async function updatePrinterTransport(
  db: D1Database,
  id: number,
  transport: "auto" | "direct" | "pi",
): Promise<void> {
  const drizzleDb = getDb(db);
  await drizzleDb.run(
    sql`UPDATE printers SET transport = ${transport} WHERE id = ${id}`,
  );
}

/** Mark a printer as broken — closes any orphan open event, inserts a new one, sets status. */
export async function markPrinterBroken(
  db: D1Database,
  id: number,
  note?: string,
): Promise<void> {
  ...
  const now = Date.now();
  await drizzleDb.run(
    sql`UPDATE printer_downtime SET ended_at = ${now} WHERE printer_id = ${id} AND ended_at IS NULL`,
  );
  await drizzleDb.run(
    sql`INSERT INTO printer_downtime (printer_id, started_at, note) VALUES (${id}, ${now}, ${note ?? null})`,
  );
  await drizzleDb.run(
    sql`UPDATE printers SET status = 'BROKEN' WHERE id = ${id}`,
  );
}

/** Mark a printer as repaired — closes the open downtime event, resets status to IDLE. */
export async function markPrinterRepaired(
  db: D1Database,
  id: number,
): Promise<void> {
  const drizzleDb = getDb(db);
  ...
  await drizzleDb.run(
    sql`UPDATE printers SET status = 'IDLE' WHERE id = ${id}`,
  );
}
...
export async function updatePrinter(
  ...
  try {
    ...
    if (printer.printerAccessCode !== undefined) {
      updates.push(
        sql`printer_access_code = ${printer.printerAccessCode || null}`,
      );
    }
    ...
    await drizzleDb.run(
      sql`UPDATE printers SET ${sql.join(updates, sql`, `)} WHERE id = ${id}`,
    );
...
export async function deletePrinter(
  ...
  try {
    ...
    if (activeJobs && activeJobs.count > 0) {
      return {
        success: false,
        error: "Cannot delete printer with active print jobs",
      };
    }
...
export async function getSpoolById(
  ...
  const drizzleDb = getDb(db);
  const result = await drizzleDb.get<Spool>(
    sql`SELECT * FROM spools WHERE id = ${id}`,
  );
  return result ?? null;
}
...
export async function updateSpoolWeight(
  ...
  const drizzleDb = getDb(db);
  await drizzleDb.run(
    sql`UPDATE spools SET remaining_weight = ${remainingWeight} WHERE id = ${id}`,
  );
}
...
export async function unloadSpool(db: D1Database, printerId: number) {
  const drizzleDb = getDb(db);
  await drizzleDb.run(
    sql`UPDATE printers SET loaded_spool_id = NULL WHERE id = ${printerId}`,
  );
}

// Load spool to printer
export async function loadSpoolToPrinter(
  db: D1Database,
  printerId: number,
  spoolId: number,
) {
  const drizzleDb = getDb(db);
  await drizzleDb.run(
    sql`UPDATE printers SET loaded_spool_id = ${spoolId} WHERE id = ${printerId}`,
  );
}
...
export async function startPrintJob(
  ...
  const module = await drizzleDb.get<PrintModule>(
    sql`SELECT * FROM print_modules WHERE id = ${moduleId}`,
  );
  ...
  return {
    ...
    expectedTime:
      typeof module.expected_time === "number"
        ? module.expected_time
        : undefined,
    expectedWeight:
      typeof module.expected_weight === "number"
        ? module.expected_weight
        : undefined,
    lowMaterial:
      spool.remaining_weight <
      (typeof module.expected_weight === "number" ? module.expected_weight : 0),
  };
}
...
export async function closeOpenPrintJobsForPrinter(
  ...
  for (const row of open ?? []) {
    ...
    const reason = isRestart
      ? "manualy restarted new job on printer"
      : "manualy started new job on printer";
    ...
    if (!isRestart && actualWeight > 0 && row.resolved_spool_id) {
      ...
      if (spool) {
        await updateSpoolWeight(
          db,
          row.resolved_spool_id,
          spool.remaining_weight - actualWeight,
        );
      }
    }
  }
...
export async function completePrintJob(
  ...
  if (
    job.status == "failed" &&
    (job.expected_time ?? 0) > job.start_time - actualEndTime
  ) {
  ...
  if (success && job.module_id) {
    const module = (await getPrintModuleById(
      db,
      job.module_id,
    )) as PrintModule | null;
...
export async function getSpoolPresetById(
  db: D1Database,
  id: number,
): Promise<SpoolPreset | null> {
  const drizzleDb = getDb(db);
  const result = await drizzleDb.get<SpoolPreset>(
...
export async function updateSpoolPreset(
  ...
  try {
    ...
    return {
      success: true,
      message: `Spool preset "${preset.name}" updated successfully`,
    };
  } catch (error) {
...
export async function deleteSpoolPreset(
  db: D1Database,
  id: number,
): Promise<ServerResponse> {
  const drizzleDb = getDb(db);
  try {
    await drizzleDb.run(
      sql`DELETE FROM module_spool_presets WHERE spool_preset_id = ${id}`,
    );
...
export async function createPrintModule(
  ...
  const presetIds =
    module.spoolPresetIds ??
    (module.defaultSpoolPresetId ? [module.defaultSpoolPresetId] : []);
...
export async function deletePrintModule(db: D1Database, moduleId: number) {
  ...
  await drizzleDb.run(
    sql`UPDATE print_jobs SET module_id = NULL WHERE module_id = ${moduleId}`,
  );
...
export async function updatePrintModule(
  ...
  if (updates.length > 0) {
    await drizzleDb.run(
      sql`UPDATE print_modules SET ${sql.join(updates, sql`, `)} WHERE id = ${id}`,
    );
  }
  ...
  if (module.spoolPresetIds !== undefined) {
    await drizzleDb.run(
      sql`DELETE FROM module_spool_presets WHERE module_id = ${id}`,
    );
    ...
    await drizzleDb.run(
      sql`UPDATE print_modules SET default_spool_preset_id = ${firstPreset} WHERE id = ${id}`,
    );
  }
...
export async function updatePrinterHours(
  db: D1Database,
  ...
  const drizzleDb = getDb(db);
  await drizzleDb.run(
    sql`UPDATE printers SET total_hours = total_hours + ${hoursToAdd} WHERE id = ${printerId}`,
  );
}
...
export async function getDefaultGridPreset(
  db: D1Database,
): Promise<GridPreset | null> {
  const drizzleDb = getDb(db);
  const result = await drizzleDb.get<GridPreset>(
    sql`SELECT * FROM grid_presets WHERE is_default = 1 LIMIT 1`,
  );
  return result ?? null;
}

export async function getGridPresetById(
  db: D1Database,
  id: number,
): Promise<GridPreset | null> {
  const drizzleDb = getDb(db);
...
export async function createGridPreset(
  db: D1Database,
  preset: NewGridPreset,
): Promise<ServerResponse> {
  const drizzleDb = getDb(db);
  try {
...
export async function updateGridPreset(
  ...
  try {
    ...
    await drizzleDb.run(
      sql`UPDATE grid_presets SET ${sql.join(updates, sql`, `)} WHERE id = ${id}`,
    );
...
export async function setDefaultGridPreset(
  db: D1Database,
  id: number,
): Promise<ServerResponse> {
  const drizzleDb = getDb(db);
  try {
    ...
    // Set the new default
    await drizzleDb.run(
      sql`UPDATE grid_presets SET is_default = 1 WHERE id = ${id}`,
    );
...
export async function markSuggestedQueueItemDone(
  ...
  const idx = queue.findIndex(
    (item) =>
      item.status !== "DONE" &&
      ((moduleIdentifier.module_id !== undefined &&
        item.module_id === moduleIdentifier.module_id) ||
        (moduleIdentifier.module_name !== undefined &&
          item.module_name === moduleIdentifier.module_name)),
  );

  if (idx === -1) {
    return {
      success: false,
      error: "No matching queue item found or already DONE",
    };
  }
  ...
  return {
    success: true,
    message: `Marked queue item as DONE: ${queue[idx].module_name}`,
  };
}
