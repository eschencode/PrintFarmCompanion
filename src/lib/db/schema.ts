...
export const printerSecrets = sqliteTable(
  "printer_secrets",
  {
    ...
    // How to send commands: 'auto' uses direct MQTT when configured (desktop),
    // otherwise standalone; 'direct' forces direct MQTT.
    transport: text("transport", { enum: ["auto", "direct"] }).default("auto").notNull(),
    createdAt: integer("created_at", { mode: "timestamp" })
      .notNull()
      .default(sql`(unixepoch())`),
    updatedAt: integer("updated_at", { mode: "timestamp" })
...
export const printJobs = sqliteTable(
  "print_jobs",
  {
    ...
    // Identifier for the physical print, including prints we didn't initiate
    // (touchscreen / SD card). Used to reconcile externally-started prints
    // detected from direct MQTT frames. UNIQUE so adoption upserts are bulletproof.
